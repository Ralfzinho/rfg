...
$title = 'Login — Racing for Glory';

// Variável para mensagem de erro simples (sem flash)
$error = '';

// Se veio POST, tenta logar
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email === '' || $senha === '') {
        ...
        // tenta autenticar usando a função auth_login do funcoes.php
        if (!auth_login($email, $senha)) {
            $error = 'E-mail ou senha inválidos.';
        } else {
            // login OK → regenera ID e redireciona
...
<section class="min-h-screen grid place-items-center bg-neutral-100">
  <div class="w-full max-w-md bg-white shadow rounded-2xl p-8">
    ...
    <?php if ($error !== ''): ?>
      <div class="mb-4 rounded-lg bg-red-50 border border-red-200 text-red-700 px-4 py-2">
    ...
    <form method="post" action="/rfg/admin-login/login.php" class="space-y-4">
      ...
      <button class="w-full bg-dark text-white rounded-lg py-2.5 hover:opacity-90">
        Entrar
      </button>
    </form>

    <p class="text-xs text-neutral-500 mt-4">
      Faça o Login para acessar as opções do painel
    </p>
  </div>
</section>
